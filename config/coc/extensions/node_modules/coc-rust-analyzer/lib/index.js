...
var Config = class {
  ...
  async onConfigChange(event) {
    ...
    if (!requiresReloadOpt)
      return;
    let reload2 = this.restartServerOnConfigChange ? true : false;
    if (!reload2) {
...
var Ctx = class {
  ...
  async startServer() {
    ...
    client.onNotification(serverStatus, async (status) => {
      ...
      if (status.health !== "ok" && ((_a = status.message) == null ? void 0 : _a.length)) {
        ...
        import_coc4.window.showWarningMessage(`rust-analyzer failed to start, run ':CocCommand rust-analyzer.reloadWorkspace' to reload`);
      }
    });
    this.client = client;
  }
  ...
  async checkUpdate(auto = true) {
    ...
    if (this.config.prompt === true) {
      ret = await import_coc4.window.showQuickpick(["Yes, download the latest rust-analyzer", "Check GitHub releases", "Cancel"], msg);
    }
    if (ret === 0) {
      if (process.platform === "win32") {
        await this.client.stop();
...
function joinLines2(ctx) {
  return async () => {
    ...
    if (mode)
      range = await import_coc5.window.getSelectedRange(mode);
    if (!range) {
...
function ssr2(ctx) {
  return async () => {
    const input = await import_coc5.workspace.callAsync("input", ["Enter request like this: foo($a, $b) ==>> ($a).foo($b): "]);
...
function debugSingle(ctx) {
  return async (runnable) => {
    ...
    const stderr_rl = import_readline.default.createInterface({
      input: proc.stderr,
      crlfDelay: Infinity
    });
...
function syntaxTree2(ctx) {
  return async () => {
    ...
    if (!ret)
      return;
    const nvim = import_coc5.workspace.nvim;
...
function explainError(ctx) {
  return async () => {
    ...
    if (diagnostic == null ? void 0 : diagnostic.code) {
      ...
      const factory = new import_coc5.FloatFactory(import_coc5.workspace.nvim);
...
async function applySnippetWorkspaceEdit(edit) {
  ...
  if (import_vscode_languageserver_protocol2.TextDocumentEdit.is(change)) {
    ...
    for (const indel of change.edits) {
      ...
      if (parsed) {
        ...
        if (placeholderLength) {
          selection = import_coc5.Range.create(startLine, startColumn, startLine, startColumn + placeholderLength);
        } else {
          position = import_coc5.Position.create(startLine, startColumn);
        }
        newText = insert;
      } else {
        lineDelta += countLines(indel.newText) - (indel.range.end.line - indel.range.start.line);
      }
      newEdits.push(import_coc5.TextEdit.replace(range, newText));
    }
    ...
    await import_coc5.workspace.applyEdit(wsEdit);
    if (selection) {
      await import_coc5.window.selectRange(selection);
    } else if (position) {
...
function openDocs2(ctx) {
  return async () => {
    ...
    if (doclink) {
      await import_coc5.commands.executeCommand("vscode.open", import_coc5.Uri.parse(doclink));
    }
  };
}
...
function peekTests(ctx) {
  return async () => {
    ...
    const locations = tests.map((it) => import_coc5.Location.create(it.runnable.location.targetUri, it.runnable.location.targetSelectionRange));
...
function moveItem2(ctx, direction) {
  return async () => {
    ...
    if (mode)
      range = await import_coc5.window.getSelectedRange(mode);
    if (!range)
      range = import_coc5.Range.create(position, position);
    const params = {
      direction,
      textDocument: { uri: document2.uri },
      range
...
`;
    const tempFile = (0, import_path4.join)((0, import_os.tmpdir)(), `${(0, import_crypto2.randomBytes)(5).toString("hex")}.html`);
